...
function configDir() {
  if (process.platform === "win32") {
    ...
    return (0, import_path.join)(appdata, "notified");
  }
  const xdg = process.env["XDG_CONFIG_HOME"] ?? (0, import_path.join)((0, import_os.homedir)(), ".config");
  return (0, import_path.join)(xdg, "notified");
}
var CONFIG_FILE = (0, import_path.join)(configDir(), "config.json");
var STATE_FILE = (0, import_path.join)(configDir(), "state.json");
function claudeDataDir() {
  return process.env["CLAUDE_CODE_DATA_DIR"] ?? (0, import_path.join)((0, import_os.homedir)(), ".claude");
}

// src/config.ts
async function loadConfig() {
  try {
    const raw = await (0, import_promises.readFile)(CONFIG_FILE, "utf8");
...
async function saveConfig(config) {
  await (0, import_promises.mkdir)((0, import_path2.dirname)(CONFIG_FILE), { recursive: true });
  const json = JSON.stringify(config, null, 2);
  await (0, import_promises.writeFile)(CONFIG_FILE, json, { mode: 384, flag: "w" });
}
...
function resolvedApiBase(config) {
...
// ../shared/dist/domain.js
var ConfidenceSchema = external_exports.enum(["high", "medium"]);
var DetectedLimitEventSchema = external_exports.object({
  reset_at: external_exports.number().int().positive(),
  source: external_exports.literal("claude_jsonl"),
  confidence: ConfidenceSchema,
  idempotency_key: external_exports.string().min(1).max(200)
});
function idempotencyKeyFor(resetAt) {
  return `reset:${resetAt}`;
}

// ../shared/dist/contracts.js
var PairStartResponseSchema = external_exports.object({
  session_id: external_exports.string().min(1),
  deep_link: external_exports.string().url(),
  qr_data: external_exports.string().min(1),
  expires_at: external_exports.number().int().positive()
});
var PairStatusResponseSchema = external_exports.discriminatedUnion("status", [
  external_exports.object({ status: external_exports.literal("pending") }),
  external_exports.object({
    status: external_exports.literal("awaiting_start"),
    child_bot_username: external_exports.string().min(1)
  }),
  external_exports.object({
    status: external_exports.literal("complete"),
    child_bot_username: external_exports.string().min(1),
    /** Present exactly once on the first poll that observes `complete`. */
    device_token: external_exports.string().min(1).optional()
  }),
  external_exports.object({ status: external_exports.literal("expired") })
]);
var NotifyRequestSchema = external_exports.object({
  reset_at_unix: external_exports.number().int().positive(),
  idempotency_key: external_exports.string().min(1).max(200)
});
var NotifyResponseSchema = external_exports.object({
  status: external_exports.enum(["scheduled", "duplicate"])
});
var TestResponseSchema = external_exports.object({
  status: external_exports.literal("sent")
});
var UnpairResponseSchema = external_exports.object({
  ok: external_exports.literal(true)
});
var HealthResponseSchema = external_exports.object({
  ok: external_exports.literal(true),
  version: external_exports.string().min(1),
  git_sha: external_exports.string().min(1)
});
...
var API_VERSION = "v1";
var PAIR_SESSION_TTL_SECONDS = 10 * 60;
var PAIR_POLL_TIMEOUT_MS = PAIR_SESSION_TTL_SECONDS * 1e3;
var PATHS = {
  health: "/health",
  pair: `/${API_VERSION}/pair`,
  pairSession: (sessionId) => `/${API_VERSION}/pair/${encodeURIComponent(sessionId)}`,
  tgManager: `/${API_VERSION}/tg/manager`,
  tgChild: (botId) => `/${API_VERSION}/tg/child/${botId}`,
...
async function request(schema, url, options = {}, timeoutMs = 8e3) {
  ...
  if (!res.ok) {
    const parsed2 = ErrorResponseSchema.safeParse(body);
    if (parsed2.success) {
      const { code, message, request_id } = parsed2.data.error;
...
function get(schema, url, token, timeoutMs) {
  return request(schema, url, { method: "GET", headers: buildHeaders(token) }, timeoutMs);
}
function post(schema, url, body, token, timeoutMs) {
  return request(
    schema,
    url,
    { method: "POST", headers: buildHeaders(token), body: body != null ? JSON.stringify(body) : void 0 },
    timeoutMs
  );
}

// src/version.ts
var PKG_VERSION = "0.1.12";
...
async function checkHookInstalled() {
  try {
    const settingsPath = (0, import_path3.join)(claudeDataDir(), "settings.json");
    const raw = await (0, import_promises2.readFile)(settingsPath, "utf8");
    ...
    return blocks.some(
      (block) => Array.isArray(block.hooks) && block.hooks.some((h) => typeof h.command === "string" && h.command.includes("_hook stop"))
    );
  } catch {
    return false;
  }
...
var HOOK_SIGNATURE = "_hook stop";
var HOOK_EVENT = "StopFailure";
var HOOK_MATCHER = "rate_limit";
var SETTINGS_FILE = () => (0, import_path4.join)(claudeDataDir(), "settings.json");
async function installHook(command) {
  ...
  if (raw !== null) {
    ...
    await (0, import_promises3.copyFile)(path, `${path}.bak`);
  }
  ...
  await (0, import_promises3.mkdir)((0, import_path4.dirname)(path), { recursive: true });
  const tmp = `${path}.tmp`;
  await (0, import_promises3.writeFile)(tmp, JSON.stringify(settings, null, 2), { mode: 384 });
...
async function uninstallHook() {
  ...
  let settings;
  try {
    settings = JSON.parse(await (0, import_promises3.readFile)(path, "utf8"));
  } catch {
    return;
  }
  ...
  if (!changed) return;
  await (0, import_promises3.writeFile)(path, JSON.stringify(settings, null, 2), { mode: 384 });
}
function hasExactCommand(settings, command) {
  ...
  return eventBlocks.some(
    (block) => Array.isArray(block.hooks) && block.hooks.some((h) => h.command === command)
  );
}
...
Bot @${status.child_bot_username} created \u2014 send /start to it in Telegram.`);
    ...
    if (status.status === "complete") {
      ...
      await saveConfig({
        ...
        paired_at: Math.floor(Date.now() / 1e3)
      });
...
var MAX_SUBMITTED = 500;
var SUBMITTED_TTL_DAYS = 14;
var EMPTY = { submitted: [], pending_submit: [] };
async function loadState() {
  try {
    const raw = await (0, import_promises4.readFile)(STATE_FILE, "utf8");
    return JSON.parse(raw);
  } catch {
    return { ...EMPTY };
  }
...
async function saveState(state) {
  const cutoff = Date.now() / 1e3 - SUBMITTED_TTL_DAYS * 86400;
  state.submitted = state.submitted.filter((e) => e.submitted_at > cutoff).slice(-MAX_SUBMITTED);
  await (0, import_promises4.mkdir)((0, import_path5.dirname)(STATE_FILE), { recursive: true });
  await (0, import_promises4.writeFile)(STATE_FILE, JSON.stringify(state, null, 2), { mode: 384, flag: "w" });
}
...
async function runUnpair() {
  ...
  const apiBase = resolvedApiBase(config);
  try {
    await post(UnpairResponseSchema, `${apiBase}/v1/unpair`, void 0, config.device_token);
  } catch (err) {
...
async function readRateLimitLines(transcriptPath) {
  const results = [];
  const rl = (0, import_readline.createInterface)({
    input: (0, import_fs.createReadStream)(transcriptPath, { encoding: "utf8" }),
    crlfDelay: Infinity
  });
  for await (const line of rl) {
    ...
    try {
      ...
      if (entry["type"] === "assistant" && entry["error"] === "rate_limit" && entry["isApiErrorMessage"] === true) {
        const msg = entry["message"];
        const content = msg?.["content"];
        const text = content?.[0]?.text;
        if (typeof text === "string" && text.includes("resets")) {
          results.push({ timestamp: entry["timestamp"], text });
        }
      }
    } catch {
    }
...
async function transcriptPathFromStdin() {
...
async function findMostRecentTranscript() {
  const claudeDir = process.env["CLAUDE_CODE_DATA_DIR"] ?? (0, import_path6.join)((0, import_os2.homedir)(), ".claude");
  const projectsDir = (0, import_path6.join)(claudeDir, "projects");
  try {
...
// src/detect/matcher.ts
var RESET_RE = /resets\s+(\d{1,2}(?::\d{2})?\s*[ap]m)\s*\(([^)]+)\)/i;
function detectRateLimit(lines) {
...
function resolveNextOccurrence(timeStr, tz, referenceMs) {
  ...
  for (let dayOffset = 0; dayOffset <= 1; dayOffset++) {
    const candidate = epochForTimeInTz(new Date(referenceMs + dayOffset * 864e5), hour, minute, tz);
    if (candidate !== null && candidate > referenceMs / 1e3 - 60) {
      return candidate;
    }
  }
  return null;
}
function epochForTimeInTz(ref, hour, minute, tz) {
  try {
    ...
    const get2 = (type) => parseInt(parts.find((p) => p.type === type).value, 10);
    ...
    return Math.floor((estimate - offset2) / 1e3);
  } catch {
    return null;
  }
}
...
async function runHookStop() {
  ...
  const alreadyDone = state.submitted.some((e) => e.idempotency_key === idempotency_key) || state.pending_submit.some((e) => e.idempotency_key === idempotency_key);
  if (alreadyDone) {
  ...
  try {
    ...
    if (res.status === "scheduled" || res.status === "duplicate") {
      state.submitted.push({
        idempotency_key,
        submitted_at: Math.floor(Date.now() / 1e3),
        reset_at
      });
    }
  } catch (err) {
...
async function flushPending(state, config, apiBase) {
  const nowSec = Math.floor(Date.now() / 1e3);
  const stillPending = [];
...
async function runHookCheckPaired() {
  if (await fileExists(CONFIG_FILE)) return;
  const pluginData = process.env["CLAUDE_PLUGIN_DATA"];
  if (!pluginData) return;
  const marker = (0, import_path7.join)(pluginData, "pair-prompted");
...
async function touchMarker(path) {
  try {
    await (0, import_promises6.mkdir)((0, import_path7.dirname)(path), { recursive: true });
    await (0, import_promises6.writeFile)(path, "");
  } catch {
  }
}

// src/redact.ts
var TOKEN_RE = /\b[A-Za-z0-9_-]{43}\b/g;
var BOT_TOKEN_RE = /\b\d{8,12}:[A-Za-z0-9_-]{30,}\b/g;
...
program2.command("unpair").description("Remove pairing, uninstall hook, delete local config").action(wrapCommand(runUnpair));
program2.command("_hook <subcommand>", { hidden: true }).description("Internal: called by Claude Code plugin hooks").action(async (subcommand) => {
  try {
    switch (subcommand) {
      case "stop":
        await runHookStop();
